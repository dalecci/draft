...
${(typeof AudioEngine !== 'undefined' && AudioEngine.vm15) ? `DEVICE MODE: VM15 vibration-plate mode is ON (${AudioEngine.vm15Mode.toUpperCase()}). This device drives a Sonic Life SW-VM15 whole-body vibration platform (3–70 Hz mechanical band, intensity dial 0–99) through its Audio-In. ${AudioEngine.vm15Mode === 'dual' ? 'DUAL mode: each frequency above 68 Hz plays at its ORIGINAL value as the carrier while its amplitude pulses at the octave-folded in-band rate — the plate physically moves at the folded rate and the original tone rides on top, both at once.' : 'FOLD mode: frequencies above 68 Hz are replaced by their octave-folded values inside the plate band.'} When designing protocols here, frequencies at or below 68 Hz are native to the plate; mention plate intensity and stance when useful; the evidence-tier band (40 Hz gamma, 7.83 Schumann, 10 Hz alpha, 25–50 Hz muscle work) needs no folding.
