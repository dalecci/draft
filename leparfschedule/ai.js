...
- lps_off_requests(id, employee_id, kind 'blockout'|'pto', date_from, date_to, reason, status pending|approved|declined|cancelled, decided_by_name, supervisor_note)   kind 'pto' is shown to people as "Vacation"; a request that was approved and later revoked is back at status 'declined' with the reason in supervisor_note
...
- lps_settings(key, value jsonb): audit_log (the change archive: every approval, revocation and edit a manager made), stores (codes, names, weekly hours), rules (minStaff, maxHoursWeek, maxDaysWeek, minShiftMin, maxShiftMin, allowFlexFill, allowCallInAnyone, neverLeaveGap, clampToStoreHours, restarts), custom_rules (array of {id,t,on,...}), store_hour_overrides, supervisor_names, supervisor_email, pin, admin_pin`;
...
const AI_TOOLS = [
  ...
  { name: "rebuild_week", risk: "danger", description: "Re-solve a whole week from templates, time off, pins and rules. Locked shifts stay. With a store, only that store is re-solved and the others are left exactly as they are. A snapshot is taken first so it can be undone.", input_schema: S(null, { week_start: str("Monday YYYY-MM-DD"), store: str("PL/PB/PV to rebuild one store only; omit or ALL for every store") }, ["week_start"]), summary: (i) => `Rebuild week of ${i.week_start} for ${!i.store || i.store === "ALL" ? "all stores" : String(i.store).toUpperCase()} (previous version saved)`, run: async (i) => { const ws = mondayOf(needDate(i.week_start)); const st = i.store ? String(i.store).toUpperCase() : "ALL"; await rebuildWeek(ws, { store: st }); return { ok: true, ...weekSummary(ws) }; } },
  { name: "build_schedule", risk: "danger", description: "Build or rebuild a run of weeks in one go — the same thing the Build / rebuild button does. Weeks that do not exist yet are created; weeks that do are re-solved. Locked shifts stay. With a store, only that store is touched.", input_schema: S(null, { week_from: str("Monday of the first week, YYYY-MM-DD (any date in that week is accepted)"), week_to: str("Monday of the last week, YYYY-MM-DD"), store: str("PL/PB/PV for one store only; omit or ALL for every store") }, ["week_from", "week_to"]),
    summary: (i) => { const a2 = mondayOf(needDate(i.week_from)), b2 = mondayOf(needDate(i.week_to)); const n = Math.max(0, Math.round(daysBetween(a2, b2) / 7) + 1); return `Build/rebuild ${n} week(s), ${weekLabel(a2)}${n > 1 ? " → " + weekLabel(b2) : ""}, for ${!i.store || i.store === "ALL" ? "all stores" : String(i.store).toUpperCase()} (each week's previous version saved)`; },
    run: async (i) => { const a2 = mondayOf(needDate(i.week_from)), b2 = mondayOf(needDate(i.week_to)); if (b2 < a2) throw new Error("week_to is before week_from"); const st = i.store ? String(i.store).toUpperCase() : "ALL"; const done = await buildRange(a2, b2, { store: st }); await logAudit({ area: "schedule", action: "rebuilt", subject: st === "ALL" ? "all stores" : store(st).name, text: `Built ${done.length} week(s) (${weekLabel(done[0].week_start)}${done.length > 1 ? " → " + weekLabel(done[done.length - 1].week_start) : ""}) for ${st === "ALL" ? "all stores" : store(st).name} from the AI command center` }); return { ok: true, weeks: done }; } },
  ...
  { name: "add_custom_rule", risk: "danger", description: `Add a custom rule. Types: ${RULE_TYPES.map((r) => `${r.t} (${r.fields.join(",")})`).join("; ")}. Fields: store = PL/PB/PV/ALL or a list of stores, day = 1..7 (1=Mon) or ANY or a list of days, emp = employee id or a list of ids (one rule can cover several people and several days), emp2 = second employee id (pair rules only), n = number, minutes = number, from/to = HH:MM, text.`, input_schema: S(null, { t: str("rule type"), store: { anyOf: [{ type: "string" }, { type: "array", items: { type: "string" } }], description: "store code, ALL, or list of store codes" }, day: { anyOf: [{ type: "string" }, { type: "array", items: { type: "string" } }], description: "1-7, ANY, or list of days" }, emp: { anyOf: [{ type: "string" }, { type: "array", items: { type: "string" } }], description: "employee id or list of employee ids" }, emp2: str("second employee id"), n: num("count/hours/days"), minutes: num("minutes"), from: str("HH:MM"), to: str("HH:MM"), text: str("note text"), order: { type: "array", items: { type: "string" }, description: "coverPriority only: employee ids/names in the order to try" }, move: bool("coverPriority only: may pull a listed person off another store's shift that day (default true)") }, ["t"]),
    summary: (i) => `Add rule: ${ruleText({ ...i, emp: i.emp ? [].concat(i.emp).map((x) => (empByRef(x) || {}).id || x) : i.emp, emp2: (empByRef(i.emp2) || {}).id || i.emp2, order: i.order ? i.order.map((x) => (empByRef(x) || {}).id || x) : i.order })} (affects all future rebuilds)`,
    run: async (i) => { if (!RULE_TYPES.some((r) => r.t === i.t)) throw new Error("unknown rule type"); const r = { id: uid(), on: true, ...i }; if (i.emp) { const ids = [].concat(i.emp).map((x) => needEmp(x).id); r.emp = ids.length === 1 ? ids[0] : ids; } if (i.emp2) r.emp2 = needEmp(i.emp2).id; if (i.order) r.order = i.order.map((x) => needEmp(x).id); if (!r.store) r.store = "ALL"; if (!r.day) r.day = "ANY"; await saveSetting("custom_rules", customRules().concat([r])); return { ok: true, rule: { id: r.id, text: ruleText(r) } }; } },
  ...
  { name: "revoke_off_request", risk: "danger", signs: true, description: "Take back time off that was already approved. It goes back to declined, the employee is told, those weeks are re-solved with them available again, and the change is written to the transparency archive.", input_schema: S(null, { request_id: str("off request id"), note: str("why — the employee sees it") }, ["request_id"]), summary: (i) => { const o = state.data.off_requests.find((x) => x.id === i.request_id); return `REVOKE approved ${o ? `${ename(o.employee_id)} ${offKindWord(o.kind)} ${fmtRangeDates(o.date_from, o.date_to)}` : i.request_id} (notifies them, rebuilds those weeks, archived)`; }, run: async (i, signer) => { const o = state.data.off_requests.find((x) => x.id === i.request_id); if (!o) throw new Error("request not found"); if (o.status !== "approved") throw new Error(`request is ${o.status}, not approved`); await revokeOff(o, signer, i.note || ""); return { ok: true, status: "declined" }; } },
  { name: "edit_off_request", risk: "danger", signs: true, description: "Change the dates, the kind or the reason of a time-off request that was already decided. The old values are kept in the transparency archive and the employee is told what changed.", input_schema: S(null, { request_id: str("off request id"), date_from: str("YYYY-MM-DD, optional"), date_to: str("YYYY-MM-DD, optional"), kind: { type: "string", enum: ["blockout", "pto"], description: "pto is Vacation" }, reason: str("optional"), note: str("note about the change") }, ["request_id"]), summary: (i) => { const o = state.data.off_requests.find((x) => x.id === i.request_id); if (!o) return `Edit ${i.request_id}`; const nf = i.date_from || o.date_from, nt = i.date_to || o.date_to, nk = i.kind || o.kind; return `EDIT ${ename(o.employee_id)}: ${offKindWord(o.kind)} ${fmtRangeDates(o.date_from, o.date_to)} → ${offKindWord(nk)} ${fmtRangeDates(nf, nt)} (notifies them, archived${o.status === "approved" ? ", rebuilds those weeks" : ""})`; }, run: async (i, signer) => { const o = state.data.off_requests.find((x) => x.id === i.request_id); if (!o) throw new Error("request not found"); const patch = { kind: i.kind || o.kind, date_from: i.date_from ? needDate(i.date_from) : o.date_from, date_to: i.date_to ? needDate(i.date_to) : o.date_to, reason: i.reason === undefined ? o.reason || "" : i.reason }; if (patch.date_to < patch.date_from) throw new Error("date_to is before date_from"); await editOffRequest(o, patch, signer, i.note || ""); return { ok: true }; } },
  { name: "revoke_swap", risk: "danger", signs: true, description: "Undo a cover or switch that was already approved. The shifts go back to their original owners and stop being locked, both people are told, and the change is written to the transparency archive.", input_schema: S(null, { swap_id: str("swap request id"), note: str("why — both people see it") }, ["swap_id"]), summary: (i) => { const s = state.data.swaps.find((x) => x.id === i.swap_id); return `UNDO approved change: ${s ? swapText(s) : i.swap_id} (shifts go back, both told, archived)`; }, run: async (i, signer) => { const s = state.data.swaps.find((x) => x.id === i.swap_id); if (!s) throw new Error("swap not found"); if (s.status !== "approved") throw new Error(`swap is ${s.status}, not approved`); await revokeSwap(s, signer, i.note || ""); return { ok: true }; } },
  { name: "read_change_archive", risk: "read", description: "The transparency archive: every approval, revocation and edit a manager made, newest first.", input_schema: S(null, { limit: num("how many, default 40") }), run: async ({ limit = 40 }) => auditLog().slice(0, Math.min(200, Math.max(1, Number(limit) || 40))) },
  { name: "decide_off_request", risk: "danger", signs: true, description: "Approve or decline a pending block-out/vacation request. Approval rebuilds the affected weeks and notifies the employee.", input_schema: S(null, { request_id: str("off request id"), approve: bool("true = approve"), note: str("note") }, ["request_id", "approve"]), summary: (i) => { const o = state.data.off_requests.find((x) => x.id === i.request_id); return `${i.approve ? "APPROVE" : "DECLINE"} ${o ? `${ename(o.employee_id)} ${offKindWord(o.kind)} ${fmtRangeDates(o.date_from, o.date_to)}` : i.request_id} (notifies them${i.approve ? ", rebuilds those weeks" : ""})`; }, run: async (i, signer) => { const o = state.data.off_requests.find((x) => x.id === i.request_id); if (!o) throw new Error("request not found"); if (o.status !== "pending") throw new Error(`request is ${o.status}`); await decideOff(o, !!i.approve, signer, i.note || ""); return { ok: true, status: state.data.off_requests.find((x) => x.id === i.request_id).status }; } },
...
async function aiProposeRule(text) {
  ...
  const prompt = `Turn this scheduling rule, written by the manager, into exactly ONE add_custom_rule tool call using the closest rule type. Use employee ids from the context. Use store codes PL, PB, PV or ALL, days 1-7 (1=Monday) or ANY, times as HH:MM. day, store and emp may be lists when the sentence names several (e.g. "Tuesday, Wednesday and Friday" → day [2,3,5]; "Selena and Maria" → emp [ids]).
...
Rule: "${text.replace(/"/g, "'")}"`;
  ...
  if (use) {
    const r = { ...use.input };
    if (r.emp) { const ids = [].concat(r.emp).map((x) => { const e = empByRef(x); if (!e) throw new Error(`Couldn't match "${x}" to an employee.`); return e.id; }); r.emp = ids.length === 1 ? ids[0] : ids; }
    if (r.order) r.order = [].concat(r.order).map((x) => { const e = empByRef(x); if (!e) throw new Error(`Couldn't match "${x}" to an employee.`); return e.id; });
